    ...
    public function saveQuestions($courseId, $questions) {
        ...
        foreach($questions as $index => $obj) {
            ...
            if ($existingQuestion == 0) {
                ...
                if ($data['video'] == '') { unset($data['video']); }
                if ($data['pdf'] == '') { unset($data['pdf']); }
                $db->insert($data);
            ...
            while ($q->getRow()) {
               ...
               //echo $q->image;
                ...
                switch($q->row['media_type']) {
                    case '0': $q->row['image_href'] = $q->image?'https://hrmaster.com.au/assets/uploads/images/'.$q->row['image']:""; break;
                    case '1': $q->row['image_href'] = $q->video?'https://hrmaster.com.au/assets/uploads/'.$q->row['video']:""; break;
                    case '3': $q->row['image_href'] = $q->pdf?'https://hrmaster.com.au/assets/uploads/'.$q->row['pdf']:""; break;
                }
